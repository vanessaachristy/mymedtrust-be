...
import { Medication, requiredAttrs } from '../../schema/Medication.model';
...
router.get("/:id", verifyToken, async (req: Request, res: Response) => {
    try {
        ...
        const recordExist = record.encryptedID && record.dataHash && record.issuerDoctorAddr !== EMPTY_ADDRESS && record.patientAddr !== EMPTY_ADDRESS && medication && medication._id;
        if (recordExist) {
            const validRecord = atob(record.dataHash) === JSON.stringify(medication.toJSON());
        ...
        if (patientExist) {
            ...
            if (!recordExist) {
                ...
                if (doctorIsWhitelisted) {
                    ...
                    const queriedObservation = await Medication.findOne({
                        _id: mongoID
                    })
